...
namespace tinyply
{
        ...
        try
        {
            ...
            while (std::getline(is, line))
            {
                ...
                if (token == "ply" || token == "PLY" || token == "")
                    ...
                    success = read_header_text(line, comments, 8);
                else if (token == "format")
                    success = read_header_format(ls);
                else if (token == "element")
                    success = read_header_element(ls);
                else if (token == "property")
                    success = read_header_property(ls);
                else if (token == "obj_info")
                    success = read_header_text(line, objInfo, 9);
                else if (token == "end_header")
                    break;
                else
                {
                    success = false;
                    break; /* unexpected header field */
                }
            }
            return success;
        }
    ...
    bool PlyFile::PlyFileImpl::read_header_format(std::istream &is)
    {
        ...
        else if (s == "ascii")
        {
            ...
            return false; // malformed header format
        return true;
    }
    ...
    bool PlyFile::PlyFileImpl::read_header_property(std::istream &is)
    {
        ...
        if (elem.properties.back().propertyType == Type::INVALID)
            return false;
        return true;
    }
    ...
    void PlyFile::PlyFileImpl::parse_data(std::istream &is, bool firstPass)
    {
                ...
                for (auto &property : element.properties)
                {
                    ...
                    if (!lookup.skip)
                    {
                        ...
                        if (firstPass)
                        {
                            ...
                            if (lookup.list_stride)
                                helper->data->list_indices[count] = listSize; // variable length support
                        }
                        else
                        {
        ...
        else
        {
            // Final callback
            if (callback)
                callback(ProgressCallbackInfo{"finished", total_bytes_parsed, total_bytes_requested});
        }
    }
