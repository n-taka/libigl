...
template<> IGL_INLINE int16_t endian_swap<int16_t, int16_t>(const int16_t & v) noexcept { uint16_t r = endian_swap<uint16_t, uint16_t>(*(uint16_t*)&v); return *(int16_t*)&r; }
template<> IGL_INLINE int32_t endian_swap<int32_t, int32_t>(const int32_t & v) noexcept { uint32_t r = endian_swap<uint32_t, uint32_t>(*(uint32_t*)&v); return *(int32_t*)&r; }
template<> IGL_INLINE int64_t endian_swap<int64_t, int64_t>(const int64_t & v) noexcept { uint64_t r = endian_swap<uint64_t, uint64_t>(*(uint64_t*)&v); return *(int64_t*)&r; }
template<> IGL_INLINE float endian_swap<uint32_t, float>(const uint32_t & v) noexcept { union { float f; uint32_t i; }; i = endian_swap<uint32_t, uint32_t>(v); return f; }
template<> IGL_INLINE double endian_swap<uint64_t, double>(const uint64_t & v) noexcept { union { double d; uint64_t i; }; i = endian_swap<uint64_t, uint64_t>(v); return d; }
...
IGL_INLINE bool PlyFile::PlyFileImpl::parse_header(std::istream & is) noexcept
{
    try
    {
        std::string line;
...
IGL_INLINE bool PlyFile::PlyFileImpl::read_header_text(std::string line, std::vector<std::string>& place, int erase)
{
    place.push_back((erase > 0) ? line.erase(0, erase) : line);
    return true;
}

IGL_INLINE bool PlyFile::PlyFileImpl::read_header_format(std::istream & is)
{
    std::string s;
    (is >> s);
...
IGL_INLINE bool PlyFile::PlyFileImpl::read_header_element(std::istream & is)
{
    elements.emplace_back(is);
    return true;
}

IGL_INLINE bool PlyFile::PlyFileImpl::read_header_property(std::istream & is)
{
...
IGL_INLINE void PlyFile::set_progress_callback(const size_t num_bytes, std::function<void(const tinyply::ProgressCallbackInfo info)> callback)
{
    impl->cb_period_bytes = num_bytes;
    impl->callback = callback; 
}
IGL_INLINE std::shared_ptr<PlyData> PlyFile::request_properties_from_element(const std::string & elementKey, const std::vector<std::string> propertyKeys, 
    const uint32_t list_size_hint)
{
    return impl->request_properties_from_element(elementKey, propertyKeys, list_size_hint);
}
IGL_INLINE void PlyFile::add_properties_to_element(const std::string & elementKey, const std::vector<std::string> propertyKeys, 
    const Type type, const size_t count, uint8_t * data, const Type listType, const size_t listCount)
{
